...
type Props = {
  pendingProposals: Completion[];
};

export function KidProposal({ pendingProposals }: Props) {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await submitKidProposal(trimmed);
      if (!res.ok) {
        setError(res.error);
  ...
  const onCancel = (id: string) => {
    setError(null);
    startTransition(async () => {
      const res = await cancelKidProposal(id);
